...
def drinkBear(*args):
    ...
    pass
    pass
# Это класс Рыцарей Круглого Стола. Рыцарь обладает свойствами (атрибутами) "имя", "легенда", "сила" и "здоровье", а ещё
# умением (методом), менять свой атрибут "здоровье" при ранениях. Внутри класса есть небольшая база данных по рыцарям, 
# выдающая рыцарям те или иные свойства в зависимости от имени.
class knight:
...
# guys = [knight('Sir Lancelot'),
        # knight('Sir Robin')]
# А это битва! Набирается список тех, кто пока жив (если жив только один - он объявляется победителем и битва переходит в
# попойку). Выводится список участников раунда и их параметры. Случайным образом назначается тот кто бьёт и тот, в кого
# прилетает. Атака!
for i in xrange(42):
    guysInFight = []
    for guy in guys: 
